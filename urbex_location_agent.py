...
def write_html_map(features: list[dict[str, Any]]) -> None:
    """Write a self-contained Leaflet page that loads discovered points."""
    feature_json = json.dumps({"type": "FeatureCollection", "features": features}, ensure_ascii=False)
    html = f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Discovered UrbEx Locations</title>
  <link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css">
  <style>
    html, body, #map {{ height: 100%; margin: 0; }}
    body {{ font-family: Arial, sans-serif; }}
    .leaflet-popup-content {{ min-width: 280px; max-width: 420px; }}
    .popup-title {{ font-weight: 700; margin-bottom: 6px; }}
    .popup-meta {{ color: #444; font-size: 12px; line-height: 1.4; }}
    .popup-links {{ display: flex; gap: 10px; flex-wrap: wrap; margin: 8px 0; font-size: 12px; }}
    .popup-links a {{ color: #0a5cad; text-decoration: none; }}
    .popup-links a:hover {{ text-decoration: underline; }}
    .popup-section {{ margin-top: 8px; }}
    .popup-section-title {{ font-size: 12px; font-weight: 700; margin-bottom: 4px; }}
    .popup-comment {{ border-top: 1px solid #ddd; padding-top: 6px; margin-top: 6px; font-size: 12px; line-height: 1.35; }}
    .popup-comment-meta {{ color: #666; font-size: 11px; margin-bottom: 3px; }}
  </style>
</head>
<body>
  <div id="map"></div>
    ...
    const layer = L.geoJSON(data, {{
      onEachFeature: (feature, marker) => {{
        ...
        const comments = evidence
          .filter((item) => item.comment || item.post_id || item.source)
          .slice(0, 6)
          .map((item) => `
            <div class="popup-comment">
              <div class="popup-comment-meta">
                ${{escapeHtml(item.username || "unknown user")}}
                ${{item.time_raw ? " | " + escapeHtml(item.time_raw) : ""}}
                ${{item.post_id ? " | " + escapeHtml(item.post_id) : ""}}
                ${{item.confidence ? " | confidence " + Number(item.confidence).toFixed(3) : ""}}
              </div>
              <div>${{escapeHtml(item.comment || "")}}</div>
              <div class="popup-links">
                ${{link(item.source, "source post/page")}}
                ${{link(item.thread_url, "thread")}}
              </div>
            </div>
          `).join("");
        const popup = `
          <div class="popup-title">${{escapeHtml(p.entity || "Location")}}</div>
          <div class="popup-meta">
            ${{escapeHtml(p.display_name || "")}}<br>
            Geocode confidence: ${{Number(p.geocode_confidence || 0).toFixed(3)}}<br>
            Agent confidence: ${{p.confidence ? Number(p.confidence).toFixed(3) : "n/a"}}
          </div>
          <div class="popup-links">
            ${{link(p.source, "source")}}
            ${{link(p.thread_url, "thread")}}
          </div>
          <div class="popup-section">
            <div class="popup-section-title">Confirming comments</div>
            ${{comments || '<div class="popup-meta">No comment excerpt stored for this pin.</div>'}}
          </div>
        `;
...
"""
    (OUTPUT_DIR / "discovered_locations_map.html").write_text(html, encoding="utf-8")
